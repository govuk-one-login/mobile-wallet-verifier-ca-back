...
import { getPrivateKey } from './adapters/node-crypto/keyAdapter';
...
export function lambdaHandlerConstructor(dependencies: IssueDocumentSigningCertificateDependencies) {
  return async (_event: unknown, context: Context) => {
    ...
    // Upload root certificate if not present
    ...
    try {
      ...
      await putObject(config.VERIFIER_KEY_BUCKET, config.VERIFIER_KEY_ID + '/privateKey.pem', privateKeyPem);
      logger.info('Private key uploaded to S3');
