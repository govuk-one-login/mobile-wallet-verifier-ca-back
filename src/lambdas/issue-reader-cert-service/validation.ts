...
import { Pkcs10CertificateRequest, X509Certificate, PublicKey } from '@peculiar/x509';
import { Logger } from '@aws-lambda-powertools/logger';
import { IssueReaderCertRequest, ErrorResponse, AttestationResult } from './types.ts';
...
export async function validateCSRContent(csrPem: string, attestedPublicKey: PublicKey): Promise<AttestationResult> {
  try {
    ...
    if (keyAlgorithm !== ANDROID_ATTESTATION_CONFIG.VALID_KEY_ALGORITHM) {
      return { valid: false, code: 'invalid_key_algorithm', message: `CSR must use ECDSA algorithm, got ${keyAlgorithm}` };
    }
    ...
    if (!ANDROID_ATTESTATION_CONFIG.VALID_ECDSA_CURVES.includes(namedCurve)) {
      return { valid: false, code: 'invalid_key_curve', message: `CSR must use approved ECDSA curve (${ANDROID_ATTESTATION_CONFIG.VALID_ECDSA_CURVES.join(', ')}), got ${namedCurve}` };
    }
