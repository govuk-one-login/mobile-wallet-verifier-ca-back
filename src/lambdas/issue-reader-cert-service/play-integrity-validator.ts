...
export function validatePlayIntegrityPayload(
  ...
  if (!expectedPackageName) {
    return { valid: false, code: 'config_error', message: 'EXPECTED_ANDROID_PACKAGE_NAME not configured' };
  }
  ...
  const hasValidDevice =
    deviceVerdicts.includes('MEETS_DEVICE_INTEGRITY') || deviceVerdicts.includes('MEETS_BASIC_INTEGRITY');
  if (!hasValidDevice) {
