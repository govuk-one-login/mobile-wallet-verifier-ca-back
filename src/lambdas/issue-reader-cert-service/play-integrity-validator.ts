...
export async function validatePlayIntegritySignature(
  token: string,
...
export function validatePlayIntegrityPayload(
  ...
  const { requestDetails, appIntegrity, deviceIntegrity, accountDetails } =
    payload;
  ...
  //Get expected paackge name from verifier app once its decided
  ...
  if (!expectedPackageName) {
    return {
      valid: false,
      code: "config_error",
      message: "EXPECTED_ANDROID_PACKAGE_NAME not configured",
    };
  }
  ...
  if (appIntegrity?.appRecognitionVerdict !== "PLAY_RECOGNIZED") {
    return {
      ...
      message: "App not recognized by Play Store",
    };
  }
  ...
  if (accountDetails?.appLicensingVerdict === "UNEVALUATED") {
    ...
    return {
      ...
      message: "App is not properly licensed",
    };
  }
